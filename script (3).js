...
function renderStages() {
    ...
    stages.forEach(stage => {
        ...
        if (!isUnlocked) {
            ...
            statusText = `يفتح في ${unlockDate.toLocaleDateString('ar-EG', { day: 'numeric', month: 'long' })}`;
        } else if (isCompleted) {
        ...
        <div class="challenge-header">
            ...
            <div style="text-align: center; margin: 1rem 0;">
                <span style="color: var(--accent-yellow); font-weight: 700;">
                    نقاطك الحالية: ${stageProgress.score}
                </span>
            </div>
        </div>
...
function showStageCompletionMessage(stage, score) {
    ...
    message += `حصلت على ${score} نقطة من هذه المرحلة\n\n`;
    
    if (completedStages === stages.length) {
        message += `أحسنت! أكملت جميع المراحل!\n`;
        message += `أنت الآن بطل التحدي\n`;
        message += `شكراً لمشاركتك في تحدي بلاكرس للروبوتات`;
    } else {
        message += `أكملت ${completedStages} مرحلة من ${stages.length}\n`;
        message += `واصل المهمة لإتمام جميع المراحل!`;
    }
...
async function showLeaderboard() {
    ...
    try {
        ...
        showLoading(false);
        alert('حدث خطأ أثناء تحميل لوحة الصدارة');
    }
}
...
async function updateParticipantsList() {
    ...
    try {
        ...
        if (players.length === 0) {
            container.innerHTML = '<p>لا يوجد مشاركون حالياً</p>';
            return;
        }
        
        let html = '<table class="leaderboard-table"><thead><tr>';
        html += '<th>الاسم</th><th>البريد</th><th>الهاتف</th><th>تاريخ التسجيل</th><th>التقدم</th><th>النقاط</th>';
        html += '</tr></thead><tbody>';
        ...
        players.forEach(player => {
            ...
            html += `<td>${player.phone || 'غير متوفر'}</td>`;
            html += `<td>${regDate}</td>`;
...
function updateCountdown() {
    ...
    if (!nextUnlock) {
        countdownEl.textContent = 'جميع المراحل متاحة الآن!';
        return;
    }
    ...
    if (diff <= 0) {
        countdownEl.textContent = 'مرحلة جديدة متاحة الآن!';
        if (currentPlayer) {
